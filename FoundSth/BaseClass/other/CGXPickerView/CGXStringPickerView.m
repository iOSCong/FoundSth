...
+ (NSArray *)ageArray:(NSArray *)arr
{
    ...
    for (NSInteger i = 1; i<=100; i++) {
        [ageArrr addObject:[NSString stringWithFormat:@"%ld",i]];
    }
    return ageArrr;
}
//年龄范围
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (!pickerLabel)
    {
        ...
        if (self.isSingleColumn) {
            pickerLabel.frame = CGRectMake(15, 0, (SCREEN_WIDTH - 30), 40);
        }else{
            pickerLabel.frame = CGRectMake(0, 0, (SCREEN_WIDTH - 30) / 3, 40);
